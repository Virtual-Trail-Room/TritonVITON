...
export async function POST(request) {
  try {
    // Parse incoming JSON. Expected fields: clothingID and imageURL.
    const body = await request.json();
    if (!body.clothingID || !body.imageURL) {
      throw new Error("Missing clothingID or imageURL in request body");
    }

    // Connect to MongoDB and insert a new clothing document with a placeholder for asset3D.
    ...
    // CLASSIFICATION STEP
    // ****************************
    // Build the absolute path to the classifier script in the "classifier" folder.
    const classifierScript = path.join(process.cwd(), 'classifier', 'predict.py');
    console.log("Classifier script path:", classifierScript);

    // Use the virtual environment's Python executable for classification.
    ...
    // Parse the classifier output to extract the predicted label.
    ...
    // We'll save it in the "public/models" directory.
